...
class neuralised_svm(SVC):
    ...
    def compute_point_pair_weights(self, x, with_intercept: bool=False, kappa=None):
        z_pos, z_neg = self.compute_z(x, with_intercept=with_intercept)
    ...
    def explain(
            ...
            reweight_explanation=False, kappa:int = 0) -> np.array:
        """
        ...
        (0, 1) standardised GI tends to perform well. If the input data is not standardised, the midpoint rule tends
        to perform better.
        ...
        :param int kappa: Unused parameter of the explanation. Used by inheriting KNN neuralisation.
        :return np.array R: The explanation for the input data x.
        """
        if with_intercept:
            raise NotImplementedError("Intercept not implemented yet")

        p_pos, p_neg = self.compute_point_pair_weights(x, with_intercept=with_intercept, kappa=kappa)

        if first_rule == "gradient":
            weighted_mean_sv_pos = np.einsum("ni, id -> nd", p_pos, self.x_sup_pos)
            weighted_mean_sv_neg = np.einsum("nj, jd -> nd", p_neg, self.x_sup_neg)
            R = 2 * (weighted_mean_sv_pos - weighted_mean_sv_neg)
        ...
        if first_rule == "GI":
            ...
            R_GI = self.explain(x, first_rule="GI", with_intercept=with_intercept, kappa=kappa)
            R_midpoint = self.explain(x, first_rule="midpoint", with_intercept=with_intercept, kappa=kappa)
    ...
    def reweight_explanation(self, R, x):
        ...
        g_pos_nb = np.log(k_pos.sum(axis=1)+1e-7)
        g_neg_nb = np.log(k_neg.sum(axis=1)+1e-7)
        ...
        g_pos = np.log(k_pos.sum(axis=1)+1e-7)
        g_neg = np.log(k_neg.sum(axis=1)+1e-7)
    ...
    def integrated_gradients(self, X, n_steps=10):
        ...
        :param n_steps: The number of steps for the integrated gradients.
        :return: The integrated gradients.
        """
        list_of_grads = []
        baseline = np.zeros_like(X)
        for i in range(n_steps):
            local_X = X + (i / n_steps) * (baseline - X)
            local_grad = self.explain(local_X, first_rule="gradient")
            list_of_grads.append(local_grad)
        avg_grad = np.mean(list_of_grads, axis=0)
        neural_IG = avg_grad * X
        return neural_IG